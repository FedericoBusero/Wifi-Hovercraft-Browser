...
//#include <Wire.h> moet niet?
GY521 sensor(GY521_I2C_ADDRESS);

#else

  #ifdef USE_FastIMU //nog deel dubbel met hierboven, kan eleganter 

    #include "FastIMU.h"
    // Nog op te zoeken hoe het volgende kan ingevuld worden vanuit de config
    LSM6DS3 IMU;  // Currently supported in FastIMU: MPU9255 MPU9250 MPU6886 MPU6500 MPU6050 ICM20689 ICM20690 BMI055 BMX055 BMI160 LSM6DS3 LSM6DSL QMI8658
    //#include <Wire.h> moet niet?
...
#else
  #ifdef USE_FastIMU //nog veel dubbel met USE_GY521
...
#else //ook hieer veel dubbel met USE_GY521
  #ifdef USE_FastIMU //ook hieer veel dubbel met USE_GY521
        // "gyro"-regeling
        const float Pfactor = GYRO_REGELING_P;
        const float max_draai_factor = GYRO_REGELING_MAX_DRAAI;
        const float bias = GYRO_REGELING_BIAS;
  ...
  */
  ...
  if (gyroBeschikbaar)
  {
    ...
    sensor.read();
...
#else
  #ifdef USE_FastIMU // bevat nog veel dubbel met #ifdef GY521, kan eleganter
      ...
      Wire.begin();
    #endif
      Wire.setClock(400000); //400khz clock //FastIMU tot commented blok
      delay(1000);

    for (int t = 0; t < 5; t++) // 5 keer proberen of gyro beschikbaar is
    {
      ...
      {
      gyroBeschikbaar = true;
      DEBUG_SERIAL.println("\tConnected to gyro");
      break;
      }
    }
      ...
      if (gyroBeschikbaar)
      {
       IMU.setGyroRange(500); // als ik het goed begrijp equivalent aan sensor.setGyroSensitivity(1);  // 500 degrees/s
       IMU.setAccelRange(8); // als ik het goed begrijp equivalent aan sensor.setAccelSensitivity(2);  // 8g
       //sensor.setThrottle(2); //Equivalent bij FastIMU nog op te zoeken

    #ifdef DEBUG_SERIAL
        DEBUG_SERIAL.println("start...");
    #endif
      // set all calibration errors to zero IMU.init(calib, IMU_I2C_ADDRESS);//nog te doen voor FastIMU
      }
...
#ifdef USE_FastIMU
// #define PERFORM_CALIBRATION //"echte" kalibratie als test
...
//     }
//     delay(5000);
//     IMU.init(calib, IMU_I2C_ADDRESS);
//     #endif

for (int t = 0; t < 50; t++) // 5 keer proberen of gyro beschikbaar is
    {
    IMU.update(); //FastIMU
    IMU.getGyro(&gyroData);// FastIMU
...
void updatestatusbar()
{
  ...
  if (currentmillis > lastupdate_voltage + TIMEOUT_MS_VOLTAGE)
  {
    ...
    //float voltage = 5.00; //voor test op ESP zonder batterij monitoring//getVoltage();
...
#endif
#ifdef USE_FastIMU
        snprintf(statusstr, sizeof(statusstr), "%4.2f F gyro:%4.2f", voltage, getGyro());
#endif
      }
      else
      {
